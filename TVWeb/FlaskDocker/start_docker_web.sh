...
sed -e "s&DNSservername&$SERVER_NAME.$DOMAIN&"  -e "s&/DOMAIN/&/$DOMAIN/&" -i /etc/nginx/nginx.conf
chown root:root  /etc/nginx/nginx.conf
echo nginx -g "daemon off;"
nginx -g "daemon off;" &

if ($debug_Flask); then
    su flaskusr -w DOMAIN,SERVER_NAME -c "/bin/bash -vx -c \"cd /TiledViz/TVWeb; FlaskDocker/launch_flask $POSTGRES_HOST $POSTGRES_PORT $POSTGRES_DB $POSTGRES_USER '$POSTGRES_PASSWORD' $flaskhost '$SECRET_KEY'\""
    while true; do sleep 10; done
else
    su - flaskusr -w DOMAIN,SERVER_NAME -c "/bin/bash -c \"cd /TiledViz/TVWeb; FlaskDocker/launch_flask $POSTGRES_HOST $POSTGRES_PORT $POSTGRES_DB $POSTGRES_USER '$POSTGRES_PASSWORD' $flaskhost '$SECRET_KEY'\""
fi
